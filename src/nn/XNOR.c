...
int main()
{
    ...
    for (int epoch = 0; epoch < epochs; ++epoch)
    {
        for (int s = 0; s < 4; ++s)
        {
            ...
            {
                double z =
                    inputs[s][0] * w1[0][j] + inputs[s][1] * w1[1][j] + b1[j];
                h[s][j] = sigmoid(z);
            }
        ...
        for (int s = 0; s < 4; ++s)
        {
            ...
            {
                delta_h[s][j] =
                    delta_o[s] * w2[j] * sigmoid_derivative(h[s][j]);
            }
        }
    ...
    {
        double hh0 =
            sigmoid(inputs[s][0] * w1[0][0] + inputs[s][1] * w1[1][0] + b1[0]);
        double hh1 =
            sigmoid(inputs[s][0] * w1[0][1] + inputs[s][1] * w1[1][1] + b1[1]);
