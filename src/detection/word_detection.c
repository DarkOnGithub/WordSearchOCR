...
bool addBoundingBox(BoundingBoxArray* array, BoundingBox box) {
    if (array->count >= array->capacity) {
        ...
        BoundingBox* new_boxes = realloc(array->boxes, new_capacity * sizeof(BoundingBox));
        if (!new_boxes) {
            return false;
        }
...
bool addWordGroup(WordGroups* groups, BoundingBoxArray* group) {
    if (groups->count >= groups->capacity) {
        ...
        BoundingBoxArray** new_groups = realloc(groups->groups, new_capacity * sizeof(BoundingBoxArray*));
        if (!new_groups) {
            return false;
        }
...
BoundingBoxArray* merge_nearby_boxes(const BoundingBoxArray* boxes, int max_gap, float max_vertical_overlap) {
    ...
    for (int i = 1; i < sorted_boxes->count; i++) {
        ...
        int y_overlap = (overlap_end > overlap_start) ? (overlap_end - overlap_start) : 0;
...
WordGroups* find_word_groups(const BoundingBoxArray* boxes, int max_distance, int alignment_threshold) {
    ...
    if (sorted_boxes->count > 0) {
        ...
        float current_y = sorted_boxes->boxes[0].y + sorted_boxes->boxes[0].height / 2.0f;
    ...
    int* gap_counts = calloc(1000, sizeof(int)); //The gap won't be more than 1000 pixels (I hope :))
    if (!gap_counts) {
        free(vertical_gaps);
        freeWordGroups(rows);
        free(rows);
    ...
    if (rows->count > 0) {
        ...
        for (int i = 1; i < rows->count; i++) {
            ...
            for (int j = 0; j < prev_row->count; j++) {
                prev_row_y += prev_row->boxes[j].y + prev_row->boxes[j].height / 2.0f;
            }
            prev_row_y /= prev_row->count;

            float current_row_y = 0.0f;
            for (int j = 0; j < current_row->count; j++) {
                current_row_y += current_row->boxes[j].y + current_row->boxes[j].height / 2.0f;
            }
...
void draw_bounding_boxes(Image* image, const BoundingBoxArray* boxes, uint32_t color, int thickness) {
    ...
    for (int i = 0; i < boxes->count; i++) {
        ...
        draw_rectangle(image, box.x, box.y, box.width, box.height, false, thickness, color);
    }
}
...
BoundingBoxArray* detect_words(const char* image_path, const char* debug_prefix) {
    ...
    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_01_original.png", debug_prefix);
    ...
    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_02_blurred.png", debug_prefix);
        save_image(debug_path, &image);
    }
    ...
    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_03_otsu_threshold.png", debug_prefix);
        save_image(debug_path, &image);
    }
    ...
    adaptiveThreshold(&original_blurred, 255, 1, 1, 11, 2); // GAUSSIAN, BINARY_INV

    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_04_adaptive_threshold.png", debug_prefix);
        save_image(debug_path, &image);
    }
    ...
    for (int i = 0; i < total_pixels; i++) {
        original_blurred2.gray_pixels[i] = (original_blurred2.gray_pixels[i] > mean_val * 0.8) ? 0 : 255;
    }

    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_05_mean_threshold.png", debug_prefix);
        save_image(debug_path, &image);
    }
    ...
    if (!combined_threshold.gray_pixels) {
        fprintf(stderr, "Error: Failed to allocate memory for combined threshold\n");
        free_image(&image);
    ...
    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_06_combined_threshold.png", debug_prefix);
    ...
    StructuringElement* kernel_small = getStructuringElement(0, 2, 2); // MORPH_RECT, 2x2
    StructuringElement* kernel_large = getStructuringElement(0, 3, 3); // MORPH_RECT, 3x3
    ...
    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_07_morphology_closed.png", debug_prefix);
        save_image(debug_path, &combined_threshold);
    }
    ...
    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_08_dilated.png", debug_prefix);
        save_image(debug_path, &combined_threshold);
    }
    ...
    if (debug_prefix) {
        char debug_path[256];
        snprintf(debug_path, sizeof(debug_path), "%s_09_eroded.png", debug_prefix);
        save_image(debug_path, &combined_threshold);
    }
    ...
    for (int i = 0; i < contours->count; i++) {
        ...
        if (boundingRect(&contours->contours[i], &rect)) {
            ...
            if (area > combined_threshold.width * combined_threshold.height * 0.15) continue;
            if (aspect_ratio < 1.5 || aspect_ratio > 15) continue;
            if (h > combined_threshold.height * 0.3) continue;
            if (w > combined_threshold.width * 0.4) continue;
            if (h < 10 || w < 15) continue;
    ...
    for (int i = 0; i < merged_boxes->count; i++) {
        ...
        if (area < 200 || box.height < 12 || box.width < 20) continue;
        if (area > image.width * image.height * 0.1 ||
            box.height > image.height * 0.25 ||
            box.width > image.width * 0.35) continue;
