...
void extract_word_images(const char* image_path, const BoundingBoxArray* words) {
    ...
    for (int i = 0; i < words->count; i++) {
        ...
        if (x < 0) x = 0;
        if (y < 0) y = 0;
        if (x + width > original_image.width) width = original_image.width - x;
        if (y + height > original_image.height) height = original_image.height - y;
        ...
        snprintf(word_filename, sizeof(word_filename), "words/%s_word_%d.png", base_name, i + 1);
...
int process_image(const char* image_path) {
    ...
    if (image.is_grayscale) {
        ...
        if (main_group->count > 1) {
            ...
            draw_rectangle(&image, min_x, min_y, max_x - min_x, max_y - min_y, false, 3, 0xFF0000FF);
        }
        ...
        snprintf(output_path, sizeof(output_path), "words_with_squares_%s.png", base_name);
