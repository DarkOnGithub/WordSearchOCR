...
int find_safe_line_position(int target_pos, int is_horizontal, int max_offset,
                           const Image* binary_img, int img_width, int img_height) {
    ...
    if (is_horizontal) {
        ...
        if (check_height <= 0) return target_pos;
        ...
        if (has_letters) {
            for (int offset = 1; offset <= max_offset; offset++) {
                ...
                if (check_start_y < 0) check_start_y = 0;
                if (check_end_y > img_height) check_end_y = img_height;

                if (check_end_y > check_start_y) {
                    int has_letters_up = 0;
                    for (int y = check_start_y; y < check_end_y && !has_letters_up; y++) {
                ...
                if (check_end_y > check_start_y) {
                    ...
                    for (int y = check_start_y; y < check_end_y && !has_letters_down; y++) {
        ...
        if (check_width <= 0) return target_pos;
        ...
        if (has_letters) {
            for (int offset = 1; offset <= max_offset; offset++) {
                ...
                if (check_start_x < 0) check_start_x = 0;
                if (check_end_x > img_width) check_end_x = img_width;

                if (check_end_x > check_start_x) {
                    int has_letters_left = 0;
                    for (int x = check_start_x; x < check_end_x && !has_letters_left; x++) {
                        for (int y = 0; y < img_height && !has_letters_left; y++) {
                            int idx = y * img_width + x;
                            if (binary_img->gray_pixels[idx] == 255) {
                                has_letters_left = 1;
                            }
                ...
                if (check_end_x > check_start_x) {
                    ...
                    for (int x = check_start_x; x < check_end_x && !has_letters_right; x++) {
                        for (int y = 0; y < img_height && !has_letters_right; y++) {
                            int idx = y * img_width + x;
                            if (binary_img->gray_pixels[idx] == 255) {
                                has_letters_right = 1;
                            }
...
Contours* filter_letter_contours(const Contours* contours) {
    ...
    for (int i = 0; i < contours->count; i++) {
        ...
        int area = bounding_rect.width * bounding_rect.height;
        double aspect_ratio = bounding_rect.width > 0 ?
            (double)bounding_rect.height / bounding_rect.width : 0.0;

        if (area > 20 && area < 5000 &&
            aspect_ratio > 0.3 && aspect_ratio < 3.0 &&
            bounding_rect.width > 8 && bounding_rect.height > 8) {
            valid_count++;
        }
    }
