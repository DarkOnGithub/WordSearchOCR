...
int main_gui(int argc, char *argv[]) {
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(header_bar), "app-header");
    gtk_style_context_add_class(gtk_widget_get_style_context(load_button), "primary-button");
    gtk_style_context_add_class(gtk_widget_get_style_context(process_button), "accent-button");

    gtk_window_set_titlebar(GTK_WINDOW(main_window), header_bar);

    paned = gtk_paned_new(GTK_ORIENTATION_HORIZONTAL);
    gtk_container_add(GTK_CONTAINER(main_window), paned);
    gtk_style_context_add_class(gtk_widget_get_style_context(paned), "main-paned");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(image_container), "panel-left");
    ...
    gtk_box_pack_start(GTK_BOX(image_container), original_section, FALSE, FALSE, 0);

    GtkWidget *original_title = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(original_title), "<b><span size='large'>Original Image</span></b>");
    gtk_style_context_add_class(gtk_widget_get_style_context(original_title), "section-title");
    gtk_box_pack_start(GTK_BOX(original_section), original_title, FALSE, FALSE, 5);
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(image_frame), "card-frame");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(results_container), "panel-right");
...
static void setup_initial_right_panel(const gchar *image_path) {
    ...
    gtk_label_set_markup(GTK_LABEL(section_title), "<b><span size='x-large'>Image Preview</span></b>");
    gtk_style_context_add_class(gtk_widget_get_style_context(section_title), "section-title");
    gtk_box_pack_start(GTK_BOX(results_container), section_title, FALSE, FALSE, 10);

    GtkWidget *right_image_frame = gtk_frame_new(NULL);
    gtk_frame_set_shadow_type(GTK_FRAME(right_image_frame), GTK_SHADOW_ETCHED_IN);
    gtk_widget_set_margin_bottom(right_image_frame, 15);
    gtk_style_context_add_class(gtk_widget_get_style_context(right_image_frame), "card-frame");
    gtk_box_pack_start(GTK_BOX(results_container), right_image_frame, FALSE, FALSE, 0);
...
static void display_right_image(const gchar *image_path) {
    ...
    if (pixbuf) {
        ...
        GdkPixbuf *scaled_pixbuf = gdk_pixbuf_scale_simple(pixbuf,
                                                         ...
                                                         new_height,
                                                         GDK_INTERP_BILINEAR);
        gtk_image_set_from_pixbuf(GTK_IMAGE(right_image_display), scaled_pixbuf);
        g_object_unref(scaled_pixbuf);
...
static void create_processing_step_button(const char* step_name, const char* filename) {
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(button), "processing-step-button");

    g_signal_connect(button, "clicked", G_CALLBACK(processing_step_callback), (gpointer)filename);
...
static void processing_step_callback(GtkWidget *widget, gpointer data) {
    ...
    if (g_file_test(filename, G_FILE_TEST_EXISTS)) {
        ...
        GtkWidget *section_title = gtk_label_new(NULL);
        gtk_label_set_markup(GTK_LABEL(section_title), "<b><span size='x-large'>Processing Step</span></b>");
        gtk_style_context_add_class(gtk_widget_get_style_context(section_title), "section-title");
        gtk_box_pack_start(GTK_BOX(results_container), section_title, FALSE, FALSE, 10);
        ...
        gtk_style_context_add_class(gtk_widget_get_style_context(step_label), "step-title");
        gtk_box_pack_start(GTK_BOX(results_container), step_label, FALSE, FALSE, 10);

        display_right_image(filename);

        gtk_widget_show_all(results_container);
    } else {
        GtkWidget *error_label = gtk_label_new("Processing step image not found.\nMake sure processing has completed.");
...
static void load_dark_theme_css(void) {
    ...
    GError *error = NULL;
    gchar *css_path = g_build_filename(g_get_current_dir(), "assets", "ui", "theme.css", NULL);
...
static void show_welcome_panel(void) {
    ...
    gtk_label_set_markup(GTK_LABEL(welcome_label), "Load an image using the button above.");
    gtk_label_set_line_wrap(GTK_LABEL(welcome_label), TRUE);
    gtk_label_set_justify(GTK_LABEL(welcome_label), GTK_JUSTIFY_CENTER);
    gtk_style_context_add_class(gtk_widget_get_style_context(welcome_label), "welcome-fallback");
    gtk_box_pack_start(GTK_BOX(results_container), welcome_label, TRUE, TRUE, 20);
...
static void process_image(const gchar *image_path) {
    ...
    if (result != 0) {
        ...
        GtkWidget *error_label = gtk_label_new("Failed to process image. Check the console for details.");
        gtk_label_set_line_wrap(GTK_LABEL(error_label), TRUE);
        gtk_box_pack_start(GTK_BOX(results_container), error_label, FALSE, FALSE, 0);
        gtk_widget_show(error_label);
    } else {
